...
import { CalendarPlusIcon, CameraIcon } from "@phosphor-icons/react";
...
export function PlanningPage() {
  ...
  return (
    <PageShell
      title="Planning & DPR"
      subtitle="Plan supervisor work, capture daily progress and track delays with photos."
      actions={<PlanSheet />}
      tabs={
    ...
    >
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_20rem]">
        <TablePanel
          title="Daily Progress Records"
          subtitle="Planned versus completed work with delay evidence."
          toolbar={
            <FilterSheetButton
              searchKey="search"
          ...
          <section className="rounded-lg border border-border/70 bg-card p-3">
            ...
            <div className="mt-3 space-y-2">
              {planningRecords.slice(0, 3).map((record) => (
                <div
                  key={record.id}
                  className="rounded-md bg-muted/35 px-2.5 py-2"
                >
                  <p className="text-sm font-semibold text-foreground">
                    {record.activity}
                  </p>
...
function PlanningLink({ href, label }: { href: string; label: string }) {
  return (
    <Link
      href={href}
      className="rounded-lg border border-border/70 bg-card px-3 py-2 text-sm font-semibold text-foreground hover:bg-muted/35 hover:text-primary"
    >
      {label}
    </Link>
  );
